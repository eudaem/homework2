...
// Test calc_asttree()
void test_calc_asttree(ASTNode* root) {
	;
}


unordered_map<vector<ASTNode>, ASTNode> asttree_table;

long long hash_vector(vector<ASTNode> raw_vector) {
	long long h = 0;
	long long a = 0;
	vector<ASTNode>::iterator it;//声明一个迭代器，来访问vector容器，作用：遍历或者指向vector容器的元素 
	for (it = raw_vector.begin(); it != raw_vector.end(); it++)
	{
		if (it->type == TYPE_DOUBLE) {
			a = floor(it->data.real);
		}
		else if (it->type == TYPE_FRACTION) {
			a = floor(it->data.frac.numerator / it->data.frac.denominator);
		}
		h = (h * 19260817 + a) % (long long)(1e9 + 7);
	}
	return h;
}

//hash_by_calc_asttree

pair<int, ASTNode> provoke_calc_asttree(ASTNode* root) {
	vector<ASTNode>Inter_results;
	ASTNode* result = new ASTNode();
	result = calc_asttree(root, Inter_results);
	pair<int, ASTNode> hash_result;
	hash_result.first = hash_vector(Inter_results);
	hash_result.second.type = result->type;
	switch (result->type)
	{
	default:
		break;
	TYPE_DOUBLE:
		hash_result.second.data.real = result->data.real;
		break;
	TYPE_FRACTION:
		hash_result.second.data.frac = result->data.frac;
		break;
	}
	return hash_result;
}
...
// calc_asttree()
ASTNode* calc_asttree(ASTNode* root, vector<ASTNode> &Inter_results) {
	ASTNode* result = new ASTNode();
	...
	case TYPE_ADD:
		...
		break;
	case TYPE_MINUS:
		temp_a = calc_asttree(root->data.node.first, Inter_results);
		temp_b = calc_asttree(root->data.node.second, Inter_results);
		if (temp_a->type == TYPE_FRACTION && temp_b->type == TYPE_FRACTION) {
			result->type = TYPE_FRACTION;
			result->data.frac = temp_a->data.frac - temp_b->data.frac;
...
// todo
void Setting(int max_opearators = 5,
	long max_range = 1000,
	long max_num = 50,
	int precision = 2,
	bool has_fraction = true,
	bool has_real = true)
{
	global_setting = { max_opearators, max_range, max_num, precision, has_fraction, has_real };
}
